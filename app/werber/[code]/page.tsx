...
async function getData(code: string) {
  ...
  const { data: rec } = await supa.from('recruiters').select('id, display_name, tiktok_handle').eq('public_code', code).single()
  if (!rec) return null
...
export default async function WerberPublic({ params }: { params: { code: string }}) {
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {data.pts.map((r:any, i:number)=> (
              <tr key={i} className="border-b"><td className="py-1 px-2">{new Date(r.created_at).toLocaleDateString()}</td><td className="px-2">{r.reason}</td><td className="px-2">{r.points}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
