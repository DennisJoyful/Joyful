...
export default async function ManagerInactivityPage() {
  const guarded = await requireManager(<ManagerInactivityInner />);
  return guarded;
}

async function ManagerInactivityInner() {
  ...
  const table = (
    <Table
      headers={['Handle','Tage','Std.','Link']}
      rows={below715.map(r => [r.handle, r.days, r.hours, <a key={r.handle} className="underline" href={r.link} target="_blank">TikTok</a>])}
    />
  );

  return (
    <div className="card">
      <h1 className="h1">Inaktivität (mein Team)</h1>
